...
if(get_bloginfo('version') >= '2.1') {
		...
		if(current_user_can('read')) {
			...
			if(file_exists(dirname(__FILE__).'/style.css') && is_readable(dirname(__FILE__).'/style.css')) {
				...
				if(!empty($oceiabar_customstyle)) {
					$oceiabar_style = str_replace(' !important', '', $oceiabar_style)."\n\n".$oceiabar_customstyle;
				}
			}
	...
	function oceiabar_js() {
		if(current_user_can('read')) {
			echo "<script type=\"text/javascript\">\n\t// <![CDATA[\n\toceiaBar = function() {\n\t\tif (document.all&&document.getElementById) {\n\t\t\tnavRoot = document.getElementById(\"oceiabar\").getElementById(\"oceiabar-quicklinks\");\n\t\t\tfor (i=0; i<navRoot.childNodes.length; i++) {\n\t\t\t\tnode = navRoot.childNodes[i];\n\t\t\t\tif (node.nodeName == \"LI\") {\n\t\t\t\t\tnode.onmouseover = function() {\n\t\t\t\t\t\tthis.className+=\" over\";\n\t\t\t\t\t}\n\t\t\t\t\tnode.onmouseout = function() {\n\t\t\t\t\t\tthis.className = this.className.replace(\" over\", \"\");\n\t\t\t\t\t}\n\t\t\t\t}\n\t\t\t}\n\t\t}\n\t}\n\twindow.onload = oceiaBar;\n\t// ]]>\n</script>\n";
		}
	}
