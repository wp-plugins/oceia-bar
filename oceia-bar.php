...
if(get_bloginfo("version") > "2.0") {
	...
	if(current_user_can("read")) {
		...
		if(is_single()) {
			...
			if(get_bloginfo("version") > "2.0.3") {
				$edit_array = array("".__("Edit this page")."", array("edit_post", $posts[0]->ID), "page.php?action=edit&#38;post=".$posts[0]->ID, "edit-link");
			} else {
		...
		// Set menu items
		if(get_bloginfo("version") > "2.0.3") {
			$menu = array(array(__("My Dashboard"), "read", "", ""), array("New Post", "edit_posts", "post-new.php", "write-link", ""), $edit_array);
		} else {
