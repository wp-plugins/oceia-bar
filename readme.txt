==========================================================================================
OCEIA BAR 2.0.9                                                               (2007/01/15)
==========================================================================================

1. WHAT'S NEW
   Oceia bar 2.0 had a smart filter to detect what directory it was being placed in (for
   displaying CSS images). Unfortunately, this wasn't functioning properly on Windows
   Server. Anyway, this has been removed, so you *must* upload the entire "oceia-bar"
   directory if you want the bar to look right.

   Also, because there were some quirks with custom themes, all CSS attributes are now
   !important. This is removed when you use custom CSS though.

   Oh and the deal with the odd version number -- starting with 2.1, Oceia bar's versions
   will try to match WordPress' version numbers. Easy, right? Also, in Oceia bar 2.1,
   WordPress 2.0.x support will be cut.
...
5. CUSTOMIZATION
   ...
   Note that if you do choose to use custom CSS, Oceia bar takes out all the !important flags
   in CSS. This doesn't matter most of the time, but it could get kind of quirky with some themes.
